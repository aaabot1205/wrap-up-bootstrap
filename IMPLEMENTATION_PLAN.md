...
## Current invocation correction (2026-09-15; outside the preserved roadmap)

[D-008](DECISIONS.md#d-008-bootstrap-and-wrap-up-require-explicit-user-invocation) supersedes automatic skill selection: both skills require an explicit user invocation. Version `2.5.0` includes shared instruction guards, Codex and Claude Code invocation metadata, configuration regressions, and Skill Creator compatibility validation. The user authorized publication and annotated tag `v2.5.0`. [STATUS.md](STATUS.md) and [HANDOFF.md](HANDOFF.md) distinguish configuration and loader evidence from the remaining native behavior checks; Antigravity enforcement is not claimed. The earlier plan body remains unchanged.
