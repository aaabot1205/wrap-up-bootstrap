...
`IMPLEMENTATION_PLAN.md` now records the agreed Phase 0 through Phase 5 roadmap. It covers versioning, automated verification and updates, optional `PROJECT_CONTEXT.yaml`, a future explicit Git publishing model, evidence-quality rules, cross-platform regression tests, and eventual macOS/Linux distribution. The plan preserves the current `wrap-up` behavior until a publishing-policy change is explicitly approved.
...
Start a new session in `C:\dev\wrap-up-bootstrap`, run `bootstrap`, and implement Phase 0 from `IMPLEMENTATION_PLAN.md`. Add version metadata and a recoverable baseline first; do not change the existing default commit/push behavior until the publishing-policy decision is explicitly approved.
