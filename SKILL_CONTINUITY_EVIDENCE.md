...
| IP-04 discovery | Verified with bounded scope | Five fresh frontmatter-only samples correctly selected bootstrap, wrap-up, or neither for positive, comparison-only, and reference-only prompts. Descriptions retain model invocation and two-field frontmatter. Host-native UI discovery was not rerun because global installation is deferred. |
...
| IP-06 review and closeout | Verified | Standards and Spec reviewers compared all 35 staged files from fixed point `a902c18a69c3dfe32bee9d56b66b63ce3c6b6f9a`. After two correction rounds, both reported no remaining P0-P3 findings. Global synchronization, version release, tag, and push remain deferred. |
...
- Real global skill synchronization and installed-copy hash comparison are `Not run` by current instruction.
- Version bump, release tag, remote push, and public release are `Not run` by current instruction.
